...
def translate_screen_to_maze(in_coords, in_size=16):
    return int(in_coords[0] / in_size), int(in_coords[1] / in_size)

def translate_maze_to_screen(in_coords, in_size=16):
...
class GameRenderer:
    def __init__(self, in_width: int, in_height: int, pacman_game):
        ...
        self._screen = pygame.display.set_mode((in_width, in_height), pygame.RESIZABLE)
    ...
    def _handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == self._ghost_respawn_event:
                self.respawn_ghosts()
    ...
    def respawn_ghosts(self):
        for ghost in self._ghosts:
            ghost.set_position(*translate_maze_to_screen(random.choice(self.pacman_game.ghost_spawns)))
            ghost.set_direction(Direction.NONE)
...
class Hero(MovableObject):
    ...
    def tick(self):
        ...
        elif self.x > self._renderer._width - self._size:
            self.x = 0
        elif self.y < 0:
            self.y = self._renderer._height - self._size
        elif self.y > self._renderer._height - self._size:
            self.y = 0
        ...
        for ghost in ghosts:
            ...
            if collides and ghost in game_objects:
                if self._renderer.is_kokoro_active():
                    ...
                    pygame.time.set_timer(self._renderer._ghost_respawn_event, 10000)  # 10s
                else:
...
if __name__ == "__main__":
    ...
    game_renderer = GameRenderer(size[0] * unified_size, size[1] * unified_size, pacman_game)
